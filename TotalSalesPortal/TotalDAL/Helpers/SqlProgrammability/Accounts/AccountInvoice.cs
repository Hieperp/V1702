...
namespace TotalDAL.Helpers.SqlProgrammability.Accounts
{
    public class AccountInvoice
    {
        ...
        private void GetPendingGoodsIssues()
        {
            ...
            queryString = queryString + "                       GoodsIssues.GoodsIssueID, GoodsIssues.Reference AS GoodsIssueReference, GoodsIssues.EntryDate AS GoodsIssueEntryDate, Customers.Code AS GoodsIssueCustomerCode, Customers.Name AS GoodsIssueCustomerName, GoodsIssues.PaymentTermID, GoodsIssues.TradePromotionID, TradePromotions.Specs AS TradePromotionSpecs, GoodsIssues.TradeDiscountRate, GoodsIssues.VATPercent, GoodsIssues.Description, GoodsIssues.Remarks " + "\r\n";            
        ...
        private void GetPendingGoodsIssueConsumers()
        {
            ...
            queryString = queryString + "                       INNER JOIN (SELECT DISTINCT CustomerID, TradePromotionID, " + (GlobalEnums.VATbyRow ? "0.0 AS" : "") + " VATPercent FROM GoodsIssues WHERE GoodsIssueID IN (SELECT GoodsIssueID FROM GoodsIssueDetails WHERE LocationID = @LocationID AND Approved = 1 AND (ROUND(Quantity - QuantityInvoice, " + (int)GlobalEnums.rndQuantity + ") > 0 OR ROUND(FreeQuantity - FreeQuantityInvoice, " + (int)GlobalEnums.rndQuantity + ") > 0))) PENDINGCustomers ON Customers.CustomerID = PENDINGCustomers.CustomerID " + "\r\n";
        ...
        private string GetPGIDsBuildSQLNew(bool isGoodsIssueID, bool isCustomerID, bool isReceiverID, bool isCommodityTypeID, bool isGoodsIssueDetailIDs)
        {
            ...
            queryString = queryString + "                   INNER JOIN GoodsIssues ON GoodsIssueDetails.GoodsIssueID = GoodsIssues.GoodsIssueID " + (!isGoodsIssueID ? " AND GoodsIssues.TradePromotionID = @TradePromotionID " : "") + "\r\n";
        ...
        private void AccountInvoiceSheet()
        {
            ...
            queryString = queryString + "       SELECT          AccountInvoices.AccountInvoiceID, AccountInvoices.EntryDate, AccountInvoices.Reference, AccountInvoices.VATInvoiceNo, AccountInvoices.VATInvoiceDate, AccountInvoices.VATInvoiceSeries, AccountInvoices.CustomerPO, " + "\r\n";

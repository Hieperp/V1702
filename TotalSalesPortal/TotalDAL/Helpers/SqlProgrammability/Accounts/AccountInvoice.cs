...
namespace TotalDAL.Helpers.SqlProgrammability.Accounts
{
    public class AccountInvoice
    {
        ...
        private void GetAccountInvoiceIndexes()
        {
            ...
            queryString = queryString + "       SELECT      AccountInvoices.AccountInvoiceID, CAST(AccountInvoices.EntryDate AS DATE) AS EntryDate, AccountInvoices.Reference, AccountInvoices.VATInvoiceNo, AccountInvoices.VATInvoiceDate, Locations.Code AS LocationCode, GoodsIssues.Reference AS GoodsIssueReference, GoodsIssues.EntryDate AS GoodsIssueEntryDate, Customers.Name AS CustomerName, Customers.BillingAddress, AccountInvoices.Description, AccountInvoices.TotalQuantity, AccountInvoices.TotalListedGrossAmount, AccountInvoices.TotalGrossAmount " + "\r\n";
        ...
        private void GetPendingGoodsIssues()
        {
            ...
            queryString = queryString + "                       GoodsIssues.GoodsIssueID, GoodsIssues.Reference AS GoodsIssueReference, GoodsIssues.EntryDate AS GoodsIssueEntryDate, GoodsIssues.PaymentTermID, GoodsIssues.VATPercent, GoodsIssues.Description, GoodsIssues.Remarks, " + "\r\n";
        ...
        private void GetPendingGoodsIssueConsumers()
        {
            ...
            queryString = queryString + "                       INNER JOIN (SELECT DISTINCT CustomerID, " + (GlobalEnums.VATbyRow ? "0.0 AS" : "") + " VATPercent FROM GoodsIssueDetails WHERE LocationID = @LocationID AND Approved = 1 AND (ROUND(Quantity - QuantityInvoice, " + (int)GlobalEnums.rndQuantity + ") > 0 OR ROUND(FreeQuantity - FreeQuantityInvoice, " + (int)GlobalEnums.rndQuantity + ") > 0)) PENDINGCustomers ON Customers.CustomerID = PENDINGCustomers.CustomerID " + "\r\n";
        ...
        private void GetPendingGoodsIssueReceivers()
        {
            ...
            queryString = queryString + "       SELECT          Receivers.CustomerID AS ReceiverID, Receivers.Code AS ReceiverCode, Receivers.Name AS ReceiverName, Receivers.VATCode AS ReceiverVATCode, Receivers.AttentionName AS ReceiverAttentionName, Receivers.Telephone AS ReceiverTelephone, Receivers.BillingAddress AS ReceiverBillingAddress, ReceiverEntireTerritories.EntireName AS ReceiverEntireTerritoryEntireName, ReceiverCategories.PaymentTermID, PENDINGReceivers.VATPercent " + "\r\n";

            queryString = queryString + "       FROM            Customers Receivers " + "\r\n";
            queryString = queryString + "                       INNER JOIN (SELECT DISTINCT ReceiverID, " + (GlobalEnums.VATbyRow ? "0.0 AS" : "") + " VATPercent FROM GoodsIssueDetails WHERE LocationID = @LocationID AND Approved = 1 AND (ROUND(Quantity - QuantityInvoice, " + (int)GlobalEnums.rndQuantity + ") > 0 OR ROUND(FreeQuantity - FreeQuantityInvoice, " + (int)GlobalEnums.rndQuantity + ") > 0)) PENDINGReceivers ON Receivers.CustomerID = PENDINGReceivers.ReceiverID " + "\r\n";
            queryString = queryString + "                       INNER JOIN EntireTerritories ReceiverEntireTerritories ON Receivers.TerritoryID = ReceiverEntireTerritories.TerritoryID " + "\r\n";
        ...
        private void GetPendingGoodsIssueDetails()
        {
            ...
            queryString = " @AccountInvoiceID Int, @LocationID Int, @GoodsIssueID Int, @CustomerID Int, @ReceiverID Int, @VATPercent decimal(18, 2), @CommodityTypeID int, @AspUserID nvarchar(128), @FromDate DateTime, @ToDate DateTime, @GoodsIssueDetailIDs varchar(3999), @IsReadonly bit " + "\r\n";
            queryString = queryString + " WITH ENCRYPTION " + "\r\n";
            queryString = queryString + " AS " + "\r\n";

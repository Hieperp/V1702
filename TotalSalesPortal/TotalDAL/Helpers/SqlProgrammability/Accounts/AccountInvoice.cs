...
namespace TotalDAL.Helpers.SqlProgrammability.Accounts
{
    public class AccountInvoice
    {
        ...
        private void GetPendingGoodsIssues()
        {
            ...
            queryString = queryString + "                      (SELECT GoodsIssueID FROM GoodsIssueDetails WHERE Approved = 1 AND ROUND(Quantity - QuantityInvoice, 0) > 0 OR ROUND(FreeQuantity - FreeQuantityInvoice, 0) > 0 " + "\r\n";
        ...
        private void GetPendingGoodsIssueConsumers()
        {
            ...
            queryString = queryString + "                              (SELECT CustomerID FROM GoodsIssueDetails WHERE LocationID = @LocationID AND Approved = 1 AND (ROUND(Quantity - QuantityInvoice, 0) > 0  OR ROUND(FreeQuantity - FreeQuantityInvoice, 0) > 0) " + "\r\n";
            queryString = queryString + "                               UNION ALL " + "\r\n";

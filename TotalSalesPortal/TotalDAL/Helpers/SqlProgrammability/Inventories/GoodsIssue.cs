...
namespace TotalDAL.Helpers.SqlProgrammability.Inventories
{
    ...
    public class GoodsIssue
    {
        ...
        private void GetGoodsIssueIndexes()
        {
            ...
            queryString = queryString + "       SELECT      GoodsIssues.GoodsIssueID, CAST(GoodsIssues.EntryDate AS DATE) AS EntryDate, GoodsIssues.Reference, Locations.Code AS LocationCode, Customers.Name AS CustomerName, CASE WHEN GoodsIssues.Addressee <> '' OR GoodsIssues.CustomerID <> GoodsIssues.ReceiverID THEN IIF(GoodsIssues.Addressee <> '', GoodsIssues.Addressee, Receivers.Name) ELSE '' END AS ReceiverDescription, GoodsIssues.DeliveryAdviceReferences, GoodsIssues.TotalQuantity, GoodsIssues.TotalFreeQuantity, GoodsIssues.TotalListedGrossAmount, GoodsIssues.TotalGrossAmount, GoodsIssues.Description, GoodsIssues.Approved " + "\r\n";

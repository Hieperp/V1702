...
namespace TotalDAL.Helpers.SqlProgrammability.Inventories
{
    ...
    public class GoodsIssue
    {
        ...
        private void GetPendingDeliveryAdvices()
        {
            ...
            queryString = queryString + " WITH ENCRYPTION " + "\r\n";
            queryString = queryString + " AS " + "\r\n";
            queryString = queryString + "       SELECT          DeliveryAdvices.DeliveryAdviceID, DeliveryAdvices.Reference AS DeliveryAdviceReference, DeliveryAdvices.EntryDate AS DeliveryAdviceEntryDate, DeliveryAdvices.PaymentTermID, DeliveryAdvices.WarehouseID, Warehouses.Code AS WarehouseCode, Warehouses.Name AS WarehouseName, DeliveryAdvices.ShippingAddress, DeliveryAdvices.VATPercent, DeliveryAdvices.Description, DeliveryAdvices.Remarks, " + "\r\n";
        ...
        private void GetPendingDeliveryAdviceCustomers()
        {
            ...
            queryString = queryString + " WITH ENCRYPTION " + "\r\n";
            queryString = queryString + " AS " + "\r\n";
            queryString = queryString + "       SELECT          CustomerCategories.PaymentTermID, CustomerReceiverPENDING.WarehouseID, Warehouses.Code AS WarehouseCode, Warehouses.Name AS WarehouseName, CustomerReceiverPENDING.ShippingAddress, CustomerReceiverPENDING.VATPercent, " + "\r\n";
            ...
            queryString = queryString + "       FROM           (SELECT DISTINCT CustomerID, ReceiverID, WarehouseID, ShippingAddress, " + (GlobalEnums.VATbyRow ? "0.0 AS" : "") + " VATPercent FROM DeliveryAdvices WHERE DeliveryAdviceID IN (SELECT DeliveryAdviceID FROM DeliveryAdviceDetails WHERE LocationID = @LocationID AND Approved = 1 AND InActive = 0 AND InActivePartial = 0 AND InActiveIssue = 0 AND (ROUND(Quantity - QuantityIssue, " + (int)GlobalEnums.rndQuantity + ") > 0 OR ROUND(FreeQuantity - FreeQuantityIssue, " + (int)GlobalEnums.rndQuantity + ") > 0))) CustomerReceiverPENDING " + "\r\n";
        ...
        {
            string queryString = " @LocationID Int, @CustomerID Int, @ReceiverID Int, @WarehouseID Int, @ShippingAddress nvarchar(200), @VATPercent decimal(18, 2) " + "\r\n";
            queryString = queryString + " WITH ENCRYPTION " + "\r\n";
            queryString = queryString + " AS " + "\r\n";

            queryString = queryString + "       SELECT DeliveryAdviceID, Reference, Description, Remarks FROM DeliveryAdvices WHERE CustomerID = @CustomerID AND ReceiverID = @ReceiverID AND WarehouseID = @WarehouseID AND ShippingAddress = @ShippingAddress " + (GlobalEnums.VATbyRow ? "" : "AND VATPercent = @VATPercent") + " AND DeliveryAdviceID IN (SELECT DeliveryAdviceID FROM DeliveryAdviceDetails WHERE LocationID = @LocationID AND Approved = 1 AND InActive = 0 AND InActivePartial = 0 AND InActiveIssue = 0 AND (ROUND(Quantity - QuantityIssue, " + (int)GlobalEnums.rndQuantity + ") > 0 OR ROUND(FreeQuantity - FreeQuantityIssue, " + (int)GlobalEnums.rndQuantity + ") > 0)) " + "\r\n";
        ...
        private void GetGoodsIssueViewDetails()
        {
            ...
            queryString = " @GoodsIssueID Int, @LocationID Int, @DeliveryAdviceID Int, @CustomerID Int, @ReceiverID Int, @WarehouseID Int, @ShippingAddress nvarchar(200), @VATPercent decimal(18, 2), @IsReadonly bit " + "\r\n";
            queryString = queryString + " WITH ENCRYPTION " + "\r\n";
            queryString = queryString + " AS " + "\r\n";

...
namespace TotalDAL.Helpers.SqlProgrammability.Inventories
{
    ...
    public class GoodsIssue
    {
        ...
        private void GetPendingDeliveryAdviceDescriptions()
        {
            ...
            queryString = queryString + "       SELECT DeliveryAdviceID, Reference, Description, Remarks FROM DeliveryAdvices WHERE CustomerID = @CustomerID AND ReceiverID = @ReceiverID AND WarehouseID = @WarehouseID AND ShippingAddress = @ShippingAddress AND (TradePromotionID = @TradePromotionID OR (TradePromotionID IS NULL AND @TradePromotionID IS NULL)) " + (GlobalEnums.VATbyRow ? "" : "AND VATPercent = @VATPercent") + " AND DeliveryAdviceID IN (SELECT DeliveryAdviceID FROM DeliveryAdviceDetails WHERE LocationID = @LocationID AND Approved = 1 AND InActive = 0 AND InActivePartial = 0 AND InActiveIssue = 0 AND (ROUND(Quantity - QuantityIssue, " + (int)GlobalEnums.rndQuantity + ") > 0 OR ROUND(FreeQuantity - FreeQuantityIssue, " + (int)GlobalEnums.rndQuantity + ") > 0)) " + "\r\n";

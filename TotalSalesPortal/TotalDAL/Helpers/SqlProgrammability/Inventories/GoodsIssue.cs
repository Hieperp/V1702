...
namespace TotalDAL.Helpers.SqlProgrammability.Inventories
{
    ...
    public class GoodsIssue
    {
        ...
        private void GetPendingDeliveryAdviceCustomers()
        {
            ...
            queryString = queryString + "       FROM           (SELECT DISTINCT CustomerID, ReceiverID, ShippingAddress, " + (GlobalEnums.VATbyRow ? "0 AS" : "") + " VATPercent FROM DeliveryAdvices WHERE DeliveryAdviceID IN (SELECT DeliveryAdviceID FROM DeliveryAdviceDetails WHERE LocationID = @LocationID AND Approved = 1 AND InActive = 0 AND InActivePartial = 0 AND InActiveIssue = 0 AND (ROUND(Quantity - QuantityIssue, " + (int)GlobalEnums.rndQuantity + ") > 0 OR ROUND(FreeQuantity - FreeQuantityIssue, " + (int)GlobalEnums.rndQuantity + ") > 0))) CustomerReceiverPENDING " + "\r\n";

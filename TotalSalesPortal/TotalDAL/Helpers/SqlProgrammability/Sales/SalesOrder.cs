...
namespace TotalDAL.Helpers.SqlProgrammability.Sales
{
    public class SalesOrder
    {
        ...
        private void GetSalesOrderIndexes()
        {
            ...
            queryString = queryString + "       SELECT      SalesOrders.SalesOrderID, CAST(SalesOrders.EntryDate AS DATE) AS EntryDate, SalesOrders.Reference, Locations.Code AS LocationCode, Customers.Name AS CustomerName, CASE WHEN SalesOrders.CustomerID = SalesOrders.ReceiverID THEN '' ELSE Receivers.Name + ', ' END + SalesOrders.ShippingAddress AS ReceiverDescription, Quotations.EntryDate AS QuotationDate, Quotations.Reference AS QuotationReference, ISNULL(VoidTypes.Name, CASE SalesOrders.InActivePartial WHEN 1 THEN N'Hủy một phần đh' ELSE N'' END) AS VoidTypeName, SalesOrders.Description, SalesOrders.TotalQuantity, SalesOrders.TotalQuantityAdvice, SalesOrders.TotalFreeQuantity, SalesOrders.TotalFreeQuantityAdvice, SalesOrders.TotalListedGrossAmount, SalesOrders.TotalGrossAmount, SalesOrders.Approved, SalesOrders.InActive, SalesOrders.InActivePartial " + "\r\n";

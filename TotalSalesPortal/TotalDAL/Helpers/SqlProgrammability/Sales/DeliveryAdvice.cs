...
namespace TotalDAL.Helpers.SqlProgrammability.Sales
{
    public class DeliveryAdvice
    {
        ...
        private void GetDeliveryAdviceIndexes()
        {
            ...
            queryString = queryString + "       SELECT      DeliveryAdvices.DeliveryAdviceID, CAST(DeliveryAdvices.EntryDate AS DATE) AS EntryDate, DeliveryAdvices.Reference, Locations.Code AS LocationCode, Customers.Name AS CustomerName, CASE WHEN DeliveryAdvices.CustomerID = DeliveryAdvices.ReceiverID THEN '' ELSE Receivers.Name + ', ' END + DeliveryAdvices.ShippingAddress AS ReceiverDescription, DeliveryAdvices.SalesOrderCodes, ISNULL(VoidTypes.Name, CASE DeliveryAdvices.InActivePartial WHEN 1 THEN N'Hủy một phần đh' ELSE N'' END) AS VoidTypeName, DeliveryAdvices.Description, DeliveryAdvices.TotalQuantity, DeliveryAdvices.TotalQuantityIssue, DeliveryAdvices.TotalFreeQuantity, DeliveryAdvices.TotalFreeQuantityIssue, DeliveryAdvices.TotalListedGrossAmount, DeliveryAdvices.TotalGrossAmount, DeliveryAdvices.Approved, DeliveryAdvices.InActive, DeliveryAdvices.InActivePartial " + "\r\n";
        ...
        private void GetCommoditiesInWarehouses(string storedProcedureName, bool withCommoditiesInGoodsReceipts, bool withCommoditiesInWarehouses, bool withCommodityTypeServices, bool getSavedData, bool includeCommoditiesOutOfStock, bool isUsingWarehouseBalancePrice)
        {
            ...
            queryString = queryString + "       IF (@@ROWCOUNT > 0) " + "\r\n";
            {
                ...
                if (!isUsingWarehouseBalancePrice) //GET 
                {
                    ...
                    queryString = queryString + "                           UNION ALL " + "\r\n";
                    queryString = queryString + "                           SELECT Promotions.PromotionID FROM Promotions INNER JOIN PromotionCustomerCategoryies ON Promotions.InActive = 0 AND Promotions.ApplyToTradeDiscount = 0 AND GetDate() >= Promotions.StartDate AND GetDate() <= Promotions.EndDate AND Promotions.PromotionID = PromotionCustomerCategoryies.PromotionID INNER JOIN AncestorCustomerCategories ON PromotionCustomerCategoryies.CustomerCategoryID = AncestorCustomerCategories.AncestorID AND AncestorCustomerCategories.CustomerCategoryID = @CustomerCategoryID " + "\r\n";
        ...
        private void GetDeliveryAdvicePendingCustomers()
        {
            ...
            queryString = queryString + "       SELECT          CustomerReceiverPENDING.PriceCategoryID, PriceCategories.Code AS PriceCategoryCode, CustomerReceiverPENDING.WarehouseID, Warehouses.Code AS WarehouseCode, Warehouses.Name AS WarehouseName, CustomerReceiverPENDING.TradeDiscountRate, CustomerReceiverPENDING.VATPercent, CustomerCategories.PaymentTermID, Customers.SalespersonID, Employees.Name AS SalespersonName, N'' AS Description, N'' AS Remarks, " + "\r\n";
        ...
        private void GetDeliveryAdvicePendingSalesOrderDetails()
        {
            ...
            queryString = " @LocationID Int, @DeliveryAdviceID Int, @SalesOrderID Int, @CustomerID Int, @ReceiverID Int, @PriceCategoryID Int, @WarehouseID Int, @ShippingAddress nvarchar(200), @TradeDiscountRate decimal(18, 2), @VATPercent decimal(18, 2), @EntryDate DateTime, @SalesOrderDetailIDs varchar(3999), @IsReadonly bit " + "\r\n";
            queryString = queryString + " WITH ENCRYPTION " + "\r\n";
            queryString = queryString + " AS " + "\r\n";

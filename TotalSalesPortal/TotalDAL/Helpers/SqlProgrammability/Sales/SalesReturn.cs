...
namespace TotalDAL.Helpers.SqlProgrammability.Sales
{
    public class SalesReturn
    {
        ...
        private void GetSalesReturnIndexes()
        {
            ...
            queryString = queryString + "       SELECT      SalesReturns.SalesReturnID, CAST(SalesReturns.EntryDate AS DATE) AS EntryDate, SalesReturns.Reference, Locations.Code AS LocationCode, Customers.Name AS CustomerName, CASE WHEN SalesReturns.CustomerID = SalesReturns.ReceiverID THEN '' ELSE Receivers.Name + ', ' END + SalesReturns.ShippingAddress AS ReceiverDescription, SalesReturns.Description, SalesReturns.TotalQuantity, SalesReturns.TotalFreeQuantity, SalesReturns.TotalListedGrossAmount, SalesReturns.TotalGrossAmount, SalesReturns.Approved " + "\r\n";
        ...
        private void GetSalesReturnPendingGoodsIssues()
        {
            ...
            queryString = queryString + "       SELECT          GoodsIssues.GoodsIssueID, GoodsIssues.Reference AS GoodsIssueReference, GoodsIssues.EntryDate AS GoodsIssueEntryDate, GoodsIssues.VATPercent, GoodsIssues.PaymentTermID, GoodsIssues.Description, GoodsIssues.Remarks, " + "\r\n";
            ...
            queryString = queryString + "                       INNER JOIN Customers ON GoodsIssues.GoodsIssueID IN (SELECT GoodsIssueID FROM GoodsIssueDetails WHERE LocationID = @LocationID AND CustomerID = @CustomerID AND ReceiverID = @ReceiverID AND EntryDate >= @FromDate AND EntryDate <= @ToDate AND Approved = 1 AND InActive = 0 AND InActivePartial = 0  AND ROUND(Quantity + FreeQuantity - QuantityReturned - FreeQuantityReturned, " + (int)GlobalEnums.rndQuantity + ") > 0) AND GoodsIssues.CustomerID = Customers.CustomerID " + "\r\n";
        ...
        private void GetSalesReturnPendingGoodsIssueDetails()
        {
            ...
            queryString = " @LocationID Int, @SalesReturnID Int, @GoodsIssueID Int, @CustomerID Int, @ReceiverID Int, @VATPercent decimal(18, 2), @FromDate DateTime, @ToDate DateTime, @GoodsIssueDetailIDs varchar(3999), @IsReadonly bit " + "\r\n";
            queryString = queryString + " WITH ENCRYPTION " + "\r\n";
            queryString = queryString + " AS " + "\r\n";
        ...
        private string BuildSQLNew(bool isGoodsIssueID, bool isGoodsIssueDetailIDs)
        {
            ...
            queryString = queryString + "       SELECT      GoodsIssues.GoodsIssueID, GoodsIssueDetails.GoodsIssueDetailID, GoodsIssues.Reference AS GoodsIssueReference, GoodsIssues.EntryDate AS GoodsIssueEntryDate, " + "\r\n";

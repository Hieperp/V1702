...
namespace TotalPortal.Areas.Accounts.APIs
{
    ...
    public class AccountInvoiceAPIsController : Controller
    {
        ...
        public JsonResult GetPendingGoodsIssueDetails([DataSourceRequest] DataSourceRequest dataSourceRequest, int? accountInvoiceID, int? goodsIssueID, int? customerID, int? receiverID, decimal? vatPercent, int? commodityTypeID, int? locationID, DateTime fromDate, DateTime toDate, string goodsIssueDetailIDs, bool isReadonly)
        {
            var result = this.accountInvoiceAPIRepository.GetPendingGoodsIssueDetails(accountInvoiceID, goodsIssueID, customerID, receiverID, vatPercent, commodityTypeID, User.Identity.GetUserId(), locationID, fromDate, toDate.AddHours(23).AddMinutes(59).AddSeconds(59), goodsIssueDetailIDs, false);
            return Json(result.ToDataSourceResult(dataSourceRequest), JsonRequestBehavior.AllowGet);
        }
